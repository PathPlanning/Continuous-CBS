...
* `<hlh_type>` - controls whether the algorithm uses high-level heuristics or not. 2 different heursitics are implemented. `0` - HL-heuristic is disabled. `1` - HL-heuristic based on solving linear programming problem (by simplex method). `2` - HL-heurstic based an greedy choose of disjoint cardinal conflicts. 
