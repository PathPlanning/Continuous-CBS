...
bool CBS::init_root(const Map &map, const Task &task, bool planforturns)
{
    CBS_Node root;
    tree.set_focal_weight(config.focal_weight);
    Path path;
    solution.low_level_expanded = 0;
    solution.low_level_expansions = 0;
    for(int i = 0; i < task.get_agents_size(); i++)
    {
        Agent agent = task.get_agent(i);
        //std::cout<<i<<" plan\n";
        path = planner.find_path(agent, map, {}, h_values, planforturns);
        if(path.cost < 0)
        {
            std::cout<<agent.id<<" "<<agent.start_id<<" "<<agent.goal_id<<" "<<h_values.get_value(agent.start_id, agent.id)<<"\n";
            return false;
        }
        solution.low_level_expanded += path.expanded;
    ...
    for(auto conflict: conflicts)
        ...
        {
            auto pathA = planner.find_path(task.get_agent(conflict.agent1), map, {get_constraint(conflict.agent1, conflict.move1, conflict.move2)}, h_values, planforturns);
            auto pathB = planner.find_path(task.get_agent(conflict.agent2), map, {get_constraint(conflict.agent2, conflict.move2, conflict.move1)}, h_values, planforturns);
            solution.low_level_expanded += pathA.expanded;
            solution.low_level_expanded += pathB.expanded;
            solution.low_level_expansions++;
            solution.low_level_expansions++;
    ...
    solution.init_conflicts = root.conflicts_num;
    tree.add_node(root);
    return true;
}
...
bool CBS::check_conflict(Move move1, Move move2)
{
    ...
    if(ctime > 0 && ctime < std::min(endTimeB,endTimeA) - startTimeA + CN_EPSILON)
        return true;
    return false;
}
...
Constraint CBS::get_wait_constraint(int agent, Move move1, Move move2)
{
    ...
    double speed = sqrt(pow(i0-i1,2) + pow(j0-j1, 2))/(move2.t2 - move2.t1);
    ...
    if(cls == 3)
    {
        ...
        interval.second = move2.t1 + (sqrt(radius*radius - dist*dist) - ha)/speed;
    }
    else if(cls == 4)
    {
        interval.first = move2.t2 - (sqrt(radius*radius - dist*dist) + sqrt(db - dist*dist))/speed;
    ...
    else if(da < radius*radius)
    {
        ...
        else
        {
            ...
            interval.first = move2.t1;
            interval.second = move2.t2 - (hb + size)/speed;
        }
    }
    else
    {
        if(db < radius*radius)
        {
            interval.first = move2.t1 + (ha - size)/speed;
            interval.second = move2.t2;
        }
        else
        {
            interval.first = move2.t1 + (ha - size)/speed;
            interval.second = move2.t1 + (ha + size)/speed;
        }
    }
...
Solution CBS::find_solution(const Map &map, const Task &task, const Config &cfg)
{
    ...
    config = cfg;
    h_values.init(map.get_size(), task.get_agents_size(), config.plan_for_turns);
    for(int i = 0; i < task.get_agents_size(); i++)
    {
        Agent agent = task.get_agent(i);
        h_values.count(map, agent);
    }
    solution.h_time = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - t);
    t = std::chrono::high_resolution_clock::now();
    int cardinal_solved = 0, semicardinal_solved = 0;
    if(!this->init_root(map, task, cfg.plan_for_turns))
        return solution;
    ...
    do
    {
        ...
        Path pathA = planner.find_path(task.get_agent(conflict.agent1), map, constraints, h_values, cfg.plan_for_turns);
        low_level_searches++;
        ...
        Path pathB = planner.find_path(task.get_agent(conflict.agent2), map, constraints, h_values, cfg.plan_for_turns);
        low_level_searches++;
        ...
        if(pathA.cost > 0)
        {
            ...
            find_new_conflicts(map, task, right, paths, pathA, conflicts, semicard_conflicts, cardinal_conflicts, low_level_searches, low_level_expanded, cfg.plan_for_turns);
            time_spent = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - time_now);
            time += time_spent.count();
            tree.add_node(right);
        }
        ...
        {
            time_now = std::chrono::high_resolution_clock::now();
            find_new_conflicts(map, task, left, paths, pathB, conflicts, semicard_conflicts, cardinal_conflicts, low_level_searches, low_level_expanded, cfg.plan_for_turns);
            time_spent = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - time_now);
            time += time_spent.count();
            tree.add_node(left);
        }
    ...
    solution.low_level_expanded /= solution.low_level_expansions;
...
void CBS::find_new_conflicts(const Map &map, const Task &task, CBS_Node &node, std::vector<Path> paths, Path path,
                             ...
                             int &low_level_searches, int &low_level_expanded, bool pft)
{
    ...
    for(auto c: new_conflicts)
    {
        ...
        if(path.agentID == c.agent1)
        {
            ...
            auto new_pathA = planner.find_path(task.get_agent(c.agent1), map, constraintsA, h_values, pft);
            constraintsB = get_constraints(&node, c.agent2);
            constraintsB.push_back(get_constraint(c.agent2, c.move2, c.move1));
            auto new_pathB = planner.find_path(task.get_agent(c.agent2), map, constraintsB, h_values, pft);
        ...
        else
        {
            ...
            auto new_pathA = planner.find_path(task.get_agent(c.agent2), map, constraintsA, h_values, pft);
            constraintsB = get_constraints(&node, c.agent1);
            constraintsB.push_back(get_constraint(c.agent1, c.move1, c.move2));
            auto new_pathB = planner.find_path(task.get_agent(c.agent1), map, constraintsB, h_values, pft);
